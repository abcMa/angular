...
- (void) getForKey:(CDVInvokedUrlCommand*)command
{
    ...
    NSString * _serviceName = [plistData valueForKey:@"SERVICE_NAME"];
    ...
    [self.commandDelegate runInBackground:^{
        ...
        if ([arguments count] >= 1)
        {
            ...
            NSString* value = [SFHFKeychainUtils getPasswordForUsername:key andServiceName:_serviceName error:&error];
...
- (void) setForKey:(CDVInvokedUrlCommand*)command
{
    ...
    [self.commandDelegate runInBackground:^{
        ...
        if ([arguments count] >= 2)
        {
            ...
            BOOL stored = [SFHFKeychainUtils storeUsername:key andPassword:value forServiceName:_serviceName updateExisting:YES error:&error];
            if (stored && error == nil) {
...
- (void) removeForKey:(CDVInvokedUrlCommand*)command
{
    ...
    [self.commandDelegate runInBackground:^{
        ...
        if ([arguments count] >= 1)
        {
            ...
            BOOL deleted = [SFHFKeychainUtils deleteItemForUsername:key andServiceName:_serviceName error:&error];
