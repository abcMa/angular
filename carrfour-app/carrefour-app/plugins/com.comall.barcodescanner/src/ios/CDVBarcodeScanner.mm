...
//------------------------------------------------------------------------------
@class CDVbcsMetaDataProcessor;
@class CDVbcsMetaDataViewController;
...
@property (nonatomic, retain) UIImageView*     readLineView;
...
// class that does the grunt work
//------------------------------------------------------------------------------
@interface CDVbcsMetaDataProcessor : NSObject <AVCaptureMetadataOutputObjectsDelegate> {}
@property (nonatomic, retain) CDVBarcodeScanner*           plugin;
@property (nonatomic, retain) NSString*                   callback;
@property (nonatomic, retain) UIViewController*           parentViewController;
@property (nonatomic, retain) CDVbcsMetaDataViewController*        viewController;
@property (nonatomic, retain) AVCaptureSession*           captureSession;
@property (nonatomic, retain) AVCaptureVideoPreviewLayer* previewLayer;
@property (nonatomic, retain) NSString*                   alternateXib;
@property (nonatomic)         BOOL                        is1D;
...
// view controller for the ui
//------------------------------------------------------------------------------
@interface CDVbcsMetaDataViewController : UIViewController <CDVBarcodeScannerOrientationDelegate> {}
@property (nonatomic, retain) CDVbcsMetaDataProcessor*  processor;
@property (nonatomic, retain) NSString*        alternateXib;
@property (nonatomic)         BOOL             shutterPressed;
@property (nonatomic, retain) IBOutlet UIView* overlayView;
// unsafe_unretained is equivalent to assign - used to prevent retain cycles in the property below
...
- (id)initWithProcessor:(CDVbcsMetaDataProcessor*)processor alternateOverlay:(NSString *)alternateXib;
- (void)startCapturing;
- (UIView*)buildOverlayView;
- (UIImage*)buildReticleImage;
- (void)shutterButtonPressed;
...
//--------------------------------------------------------------------------
- (void)scan:(CDVInvokedUrlCommand*)command {
    CDVbcsMetaDataProcessor* metaDataprocessor;
    ...
    if(isIOS7) {
        NSLog(@"ios7使用原生api...");
        metaDataprocessor = [[CDVbcsMetaDataProcessor alloc]
                     initWithPlugin:self
                     callback:callback
                     parentViewController:self.viewController
                     alterateOverlayXib:overlayXib
                     ];
        [metaDataprocessor retain];
        [metaDataprocessor retain];
        [metaDataprocessor retain];
        // queue [processor scanBarcode] to run on the event loop
        [metaDataprocessor performSelector:@selector(scanBarcode) withObject:nil afterDelay:0];
    }else{
        NSLog(@"ios以下使用zxing...");
...
// class that does the grunt work
//------------------------------------------------------------------------------
@implementation CDVbcsMetaDataProcessor
...
- (void)scanBarcode {
    ...
    self.viewController = [[[CDVbcsMetaDataViewController alloc] initWithProcessor: self alternateOverlay:self.alternateXib] autorelease];
...
-(void)captureOutput:(AVCaptureOutput *)captureOutput didOutputMetadataObjects:(NSArray *)metadataObjects fromConnection:(AVCaptureConnection *)connection {
    ...
    if (metadataObjects.count>0) {
        ...
        // 条码格式 todo
        // NSString*  format =  @"QR_CODE";
        NSString*  format =  metadataObject.type;
...
// view controller for the ui
//------------------------------------------------------------------------------
@implementation CDVbcsMetaDataViewController
@synthesize processor      = _processor;
@synthesize shutterPressed = _shutterPressed;
@synthesize alternateXib   = _alternateXib;
@synthesize overlayView    = _overlayView;
@synthesize readLineView    = _readLineView;
...
- (UIView*)buildOverlayView {
    ...
    [toolbar setShadowImage:[[[UIImage alloc] init] autorelease] forToolbarPosition:UIToolbarPositionAny];

    NSString* imagePath = [[NSBundle mainBundle] pathForResource:@"nav_back" ofType:@"png"];
    UIImage *cancelButtonImage = [UIImage imageWithContentsOfFile:imagePath];
    UIBarButtonItem* cancelButton = [[[UIBarButtonItem alloc] autorelease]
                                     initWithImage:cancelButtonImage
    ...
    UIBarButtonItem *negativeSeparator = [[[UIBarButtonItem alloc] autorelease]
                                          initWithBarButtonSystemItem:UIBarButtonSystemItemFixedSpace
                                          target:nil
                                          action:nil];
    negativeSeparator.width = -10;

    UILabel* titleLabel = [[UILabel alloc] initWithFrame:CGRectMake(0.0f, 0.0f, 80.0f, NAV_HEIGHT_NO_STATUSVIEW)];
...
#else
    toolbar.items = [NSArray arrayWithObjects:negativeSeparator,cancelButton,flexSpace,barTitle,flexSpace,nil];
#endif
    ...
    CGRect  rectArea       = CGRectMake(0, NAV_START_Y, rootViewWidth, NAV_HEIGHT_NO_STATUSVIEW);
    [toolbar setFrame:rectArea];

    UIView* navBar = [[[UIView alloc]
    ...
    imagePath = [[NSBundle mainBundle] pathForResource:@"bar-background" ofType:@"png"];
    UIImage* backgroundImage = [UIImage imageWithContentsOfFile:imagePath];
    [navBar setBackgroundColor:[UIColor colorWithPatternImage:backgroundImage]];
    ...
    CGFloat horizontalOffset = 0.5 * rootViewWidth - 0.25 * minAxis;
    CGFloat verticalOffset = 0.5 * rootViewHeight - 0.25 * minAxis;

    UIColor* maskColor = [UIColor colorWithRed:0.0 green:0.0 blue:0.0 alpha:0.375];
    UIImageView* topMaskView = [[[UIImageView alloc]
                                 initWithFrame:CGRectMake(
                                                          0,
                                                          0,
                                                          rootViewWidth,
    ...
    [overlayView addSubview: topMaskView];

    UIImageView* bottomMaskView = [[[UIImageView alloc]
                                    initWithFrame:CGRectMake(
                                                             0,
                                                             rootViewHeight - verticalOffset,
                                                             rootViewWidth,
                                                             verticalOffset
                                                             )
                                    ] autorelease];
    bottomMaskView.backgroundColor = maskColor;
    [overlayView addSubview: bottomMaskView];

    UIImageView* leftMaskView = [[[UIImageView alloc]
                                  initWithFrame:CGRectMake(
                                                           0,
                                                           verticalOffset,
                                                           0.25* minAxis,
    ...
    [overlayView addSubview: leftMaskView];

    UIImageView* rightMaskView = [[[UIImageView alloc]
                                   initWithFrame:CGRectMake(
                                                            rootViewWidth - horizontalOffset,
                                                            verticalOffset,
                                                            0.25* minAxis,
                                                            0.5 * minAxis
                                                            )
    ...
    [overlayView addSubview: rightMaskView];

    [self loopDrawLine:minAxis/2-20 forDistance:minAxis/2-20 withLeftOffset:horizontalOffset+10 withTopOffset:verticalOffset+10];
...
#define RETICLE_OFFSET   125.0f
#define RETICLE_ALPHA     0.4f
#define RETICLE_LENGTH     30.0f
...
- (UIImage*)buildReticleImage {
    ...
    if (self.processor.is2D) {
        ...
        // top left corner
        CGContextMoveToPoint(context, RETICLE_OFFSET+RETICLE_WIDTH/2, RETICLE_OFFSET);
        CGContextAddLineToPoint(context, RETICLE_OFFSET+RETICLE_WIDTH/2, RETICLE_OFFSET+RETICLE_LENGTH);
        CGContextMoveToPoint(context, RETICLE_OFFSET, RETICLE_OFFSET+RETICLE_WIDTH/2);
        CGContextAddLineToPoint(context, RETICLE_OFFSET+RETICLE_LENGTH, RETICLE_OFFSET+RETICLE_WIDTH/2);

        // bottom left corner
        CGContextMoveToPoint(context, RETICLE_OFFSET+RETICLE_WIDTH/2, RETICLE_SIZE-RETICLE_OFFSET);
        CGContextAddLineToPoint(context, RETICLE_OFFSET+RETICLE_WIDTH/2, RETICLE_SIZE-RETICLE_OFFSET-RETICLE_LENGTH);
        CGContextMoveToPoint(context, RETICLE_OFFSET, RETICLE_SIZE-RETICLE_OFFSET-RETICLE_WIDTH/2);
        CGContextAddLineToPoint(context, RETICLE_OFFSET+RETICLE_LENGTH, RETICLE_SIZE-RETICLE_OFFSET-RETICLE_WIDTH/2);

        // bottom right corner
        CGContextMoveToPoint(context, RETICLE_SIZE-RETICLE_OFFSET-RETICLE_WIDTH/2, RETICLE_SIZE-RETICLE_OFFSET);
        CGContextAddLineToPoint(context, RETICLE_SIZE-RETICLE_OFFSET-RETICLE_WIDTH/2, RETICLE_SIZE-RETICLE_OFFSET-RETICLE_LENGTH);
        CGContextMoveToPoint(context, RETICLE_SIZE-RETICLE_OFFSET, RETICLE_SIZE-RETICLE_OFFSET-RETICLE_WIDTH/2);
        CGContextAddLineToPoint(context, RETICLE_SIZE-RETICLE_OFFSET-RETICLE_LENGTH, RETICLE_SIZE-RETICLE_OFFSET-RETICLE_WIDTH/2);

        // top right corner
        CGContextMoveToPoint(context, RETICLE_SIZE-RETICLE_OFFSET-RETICLE_WIDTH/2, RETICLE_OFFSET);
        CGContextAddLineToPoint(context, RETICLE_SIZE-RETICLE_OFFSET-RETICLE_WIDTH/2, RETICLE_OFFSET+RETICLE_LENGTH);
        CGContextMoveToPoint(context,RETICLE_SIZE-RETICLE_OFFSET, RETICLE_OFFSET+RETICLE_WIDTH/2);
        CGContextAddLineToPoint(context, RETICLE_SIZE-RETICLE_OFFSET-RETICLE_LENGTH, RETICLE_OFFSET+RETICLE_WIDTH/2);
...
-(void)loopDrawLine:(CGFloat)length forDistance:(CGFloat)distance withLeftOffset:(CGFloat)left withTopOffset:(CGFloat)top
{
    CGRect rect = CGRectMake(left, top, length, 2);
    if (_readLineView) {
        [_readLineView removeFromSuperview];
    }
    _readLineView = [[[UIImageView alloc] initWithFrame:rect] autorelease];
    _readLineView.backgroundColor = [UIColor colorWithRed:0.0 green:1.0 blue:0.0 alpha:RETICLE_ALPHA/2];
    [UIView animateWithDuration: 2.0
                          delay: 0.0
                        options: UIViewAnimationOptionCurveEaseIn
                     animations: ^{
                         //修改fream的代码写在这里
                         _readLineView.frame =CGRectMake(left, top+distance, length, 2);
                         [_readLineView setAnimationRepeatCount:0];

                     }
                     completion: ^(BOOL finished){
                         [self loopDrawLine:length forDistance:distance withLeftOffset:left withTopOffset:top];
                     }];
