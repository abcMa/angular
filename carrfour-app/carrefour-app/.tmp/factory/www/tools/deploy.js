 ...
 * 热部署
 */
angular.module('app.services').factory('deploy', ["api", "$timeout", function (
    api, $timeout
) {
    ...
    var deployService = {
        ...
        update: function () {
            ...
            return self.check()
                .then(function (response) {
                    console.info('Deploy: update - has update - ' + response.update);
                    if (response.update) {
        ...
        // 2016-10-19
        // 增加进度汇报方式
        downloadByProcess: function downloadByProcess(updateProcessCallback) {
            if (!deploy) return api.reject(NO_PLUGIN);
            ...
            deploy.download(function (result) {
                ...
                updateProcessCallback.call(this, result);
            }, function (error) {
        ...
        // 2016-10-19
        // 增加进度汇报方式
        extractByProcess: function extractByProcess(updateProcessCallback) {
            if (!deploy) return api.reject(NO_PLUGIN);
         ...
         */
        watch: function watch(options, updateCallback) {
            ...
            function checkForUpdates() {
                this.check().then(function (response) {
                    if (response.update) {
