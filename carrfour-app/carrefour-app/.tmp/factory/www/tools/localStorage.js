...
angular.module('app.services').provider('localStorage', function() {
    ...
    // @ngInject
    provider.$get = function($window, messageCenter) {
        var localStorage = {
            /**
             * 判断数据是否存在
             */
            ...
            clean: function() {
                ...
                // 该事件将在 localStorage 清空之后发布
                messageCenter.publishMessage('localStorage.clean');
            }
        };

        return localStorage;
    };
    provider.$get.$inject = ["$window", "messageCenter"];
});
