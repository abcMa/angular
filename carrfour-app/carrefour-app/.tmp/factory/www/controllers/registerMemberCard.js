 ...
 * 注册会员卡
 */
angular.module('app.controllers').controller('registerMemberCardCtrl', ["$scope", "$params", "modals", "errorHandling", "toast", "loading", "cmSwitcherDelegate", "globalService", "$timeout", "$interval", "memberCenterService", "$translate", "dateFilter", "messageCenter", "$ionicPlatform", "$state", "identityCodeValid", function (
    $scope, $params, modals, errorHandling, toast, loading, cmSwitcherDelegate, globalService,
    $timeout, $interval, memberCenterService, $translate, dateFilter, messageCenter, $ionicPlatform, $state, identityCodeValid
) {
    var ctrl = this;
    ...
    var switcher = cmSwitcherDelegate.$getByHandle('register-member-card-modal');
    ...
    _.assign(ctrl, {
        ...
        init: function () {
            memberCenterService.getRegionRegister().success(function(data){
                ctrl.formData.regionIds = data;
            }).error(errorHandling)
            .error(function () {
                loading.close();
            });
            globalService.getArticleByStructure('registerMemberCard')
        ...
        getRegisterVerifyCode: function () {
            loading.open();
            ctrl.countdown = 90;
            memberCenterService.getRegisterVerifyCode(ctrl.formData.mobile)
                .error(errorHandling)
                .finally(function () {
        ...
        onNextStep: function () {
            ...
            // 身份证截取生日
            var idNumberLength = ctrl.formData.idNumber.length;
            ctrl.formData.idNumber = ctrl.formData.idNumber.replace(/\s/g, "");
            if((idNumberLength == 18) && (ctrl.formData.idType == 0)){
                var birthday = ctrl.formData.idNumber.substring(6,14);
                if(/^\d{8}$/.test(birthday)){
                    ctrl.formData.birthday = birthday.substring(0, 4) + "-" + birthday.substring(4, 6) + "-" + birthday.substring(6, 8);
                }
            }

            // 身份证截取性别
            if( idNumberLength == 18 && ctrl.formData.idType == 0){
                var sexNum = ctrl.formData.idNumber[ctrl.formData.idNumber.length - 2]%2;
                sexNum == 0 ? ctrl.formData.sex = '2' : ctrl.formData.sex = '1';
            } else if(idNumberLength == 15 && ctrl.formData.idType == 0){
                var sexNum2 = ctrl.formData.idNumber[ctrl.formData.idNumber.length - 1]%2;
                sexNum2 == 0 ? ctrl.formData.sex = '2' : ctrl.formData.sex = '1';
            }

            loading.open();
            memberCenterService.registerMemberCardCheckMobile(ctrl.formData.mobile, ctrl.formData.verifyCode, ctrl.formData.nationality, ctrl.formData.idType, ctrl.formData.idNumber)
                .success(function () {
                    switcher.next();
                })
                .error(errorHandling)
        ...
        close: function(){
            if(!switcher.getIsFirst()){
                ...
                modals.registerMemberCard.close();
                debindBackButtonAction();
            }
        },

        validatIdNumber: function(text){
            if(ctrl.formData.idType != 0) return;
            if (!text || !text.length){
                toast.open($translate.instant('memberCenterFormFields.errorIdNumber1'));
            } else {
                identityCodeValid.identityCodeValid(text);
            }
        },
        ...
        onSubmit: function () {
            loading.open();
            memberCenterService.registerMemberCard(ctrl.formData)
                .success(function () {
                    toast.open($translate.instant('registerMemberCard.success'));
    ...
    // 物理键返回
    var debindBackButtonAction = $ionicPlatform.registerBackButtonAction(function (e) {
        e.preventDefault();
        ctrl.close();
    }, 501);
    debindBackButtonAction();
