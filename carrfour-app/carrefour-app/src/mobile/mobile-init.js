...
// 初始化极光推送
app.run(function ($rootScope, jPushService) {
...
// 启动弹出app引导评价
app.run(function (feedbackService) {
...
// 初始化弹层活动页
app.run(function (messageCenter, activityService, modals) {
    //首页文章弹层
...
// 应用启动时打开欢迎页
app.run(function (modals, localStorage, indexService, globalService, $ionicPlatform, messageCenter, $timeout) {
...
// 初始化数据统计
app.run(function (analytics, messageCenter) {
    analytics.init(APP_CONFIG.atAnalyticSiteId, false);
});

// 版本检测
app.run(function (localStorage, modals, globalService, popup) {
...
app.run(function (popup, networkStatus, deploy, messageCenter) {
...
//初始化 数据统计
app.run(function (cofferSem, localStorage) {
