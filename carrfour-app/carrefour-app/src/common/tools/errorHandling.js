    ...
    provider.$get = function(toast, modals, $translate) {
        ...
        var defaultErrorHandlers = {
            ...
            other: function(data, status, headers, config, designees) {
                ...
                if (!message && code) {
                    var translatePath = 'stateTexts.' + APP_STATE_CODE[code];

                    message = $translate.instant(translatePath);

                    if (message === translatePath) {
                        message = undefined;
                    }
                }
        ...
        // 将默认错误处理器暴露给外部使用，但原则上仅供自定义错误处理器调用默认错误处理器使用。
        errorHandling.getDefaultErrorHandlers = function(name) {
            return defaultErrorHandlers[name];
        };
